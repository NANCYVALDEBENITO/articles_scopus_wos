...
##Writing the number of articles with DOI in WOS 
...
df = pd.read_csv("input_table/02_webofscience.csv", sep='\t', index_col=['DI'], encoding='utf-8',error_bad_lines=False)
print df
#print df["C1"].head()

df["AFF"] = df["C1"].map(parseaff)
df["NAT"] = df["C1"].map(parseafftot)
df["INST"]= df["C1"].map(parseinst)

df_out = pd.DataFrame(df, columns = ['AFF','INST','TI','NAT'])
#print df.head()

# output: DOI + AFF, to mix with Masterlist
df_out.to_csv(str("output_table/05_ARTICLES_AFF.csv"), sep='\t', encoding='utf-8')
